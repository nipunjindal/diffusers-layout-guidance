...
class LayoutGuidanceStableDiffusionPipeline(StableDiffusionPipeline):
    def __init__(
        self,
        vae: AutoencoderKL,
    ...
    @staticmethod
    def _layout_index(prompt: str, layout_info: Dict[str, Tuple[float, float, float, float]]) -> List[List[int]]:
        prompt_list = prompt.strip('.').split(' ')
        object_positions = []
        for obj in layout_info.keys():
            obj_words = obj.split(' ')
    ...
    def __call__(
        ...
        layout_info: Dict[str, Tuple[float, float, float, float]] = None
    ):
        r"""
        ...
        #TODO: NJ
        layout_index = self._layout_index(prompt, layout_info)
        print(layout_index)
